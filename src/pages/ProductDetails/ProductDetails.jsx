    ...
    useEffect(() => {
        ...
        const handleSubProducts = () => {
            if (item && item.subCategory) {
                axios.get(`https://backend-kappa-beige.vercel.app/product?subCategory=${item.subCategory}`)
                    .then((res) => {
            ...
            <div className="row my-5 row-gap-3">
                <div className="col-12 col-md-6 d-flex flex-md-row flex-column row-gap-2">
                    <div className="col-4 d-flex flex-row  flex-md-column align-items-center gap-4 gap-md-4  order-1 order-md-0">
                        <img onClick={() => setimage("one")} src={item.defaultImage.url} alt='img' className={style.smallImg} />
                        <img onClick={() => setimage("two")} src={item.image[0].url} alt='img' className={style.smallImg} />
                        <img onClick={() => setimage("three")} src={item.image[1].url} alt='img' className={style.smallImg} />
                    </div>
                ...
                <div className="col-12 col-md-6 d-flex flex-column justify-content-between gap-2">
                    ...
                    <h5>price : <span className='fs-3'>{item.finalPrice} eg</span></h5>
                    <div className="d-flex align-items-center gap-3">
                        <span className='fs-5'>Qty : </span>
                        <span className="border border-black rounded-1 d-flex" style={{ width: 'fit-content' }}>
                    ...
                    <div className="border-top p-2">
                        <p className='m-0 fs-5'>category : <span className='fs-4'>islamic Products</span></p>
                    </div>
                </div>
            </div>
            <br />
                    ...
                    {Products.map((x, ind) => {
                        return (
                            ...
                            <SwiperSlide className={style.swiper_slide}>
                                <ProductCard ProdImage={x.defaultImage.url} secondImage={x.image[0].url} name={x.name} descripe={x.description} price={x.finalPrice} oldprice={x.price} prodId={x.id} />
                            </SwiperSlide>
                        )
                    })}
                </Swiper>
                ...
                <motion.div
                    ...
                    >
                        <SwiperSlide className='position-relative'>
                            <p onClick={() => setappear(false)} className='position-absolute ' style={{ top: '2vh', right: '2vw', cursor: 'pointer' }}><IoIosCloseCircleOutline size={45} /></p>
                            <img src={item.defaultImage.url} alt="img" width='100%' height='100%' />
                        </SwiperSlide>
                        <SwiperSlide >
                            <p onClick={() => setappear(false)} className='position-absolute ' style={{ top: '2vh', right: '2vw', cursor: 'pointer' }}><IoIosCloseCircleOutline size={45} /></p>
                            <img src={item.image[0].url} alt="img" width='100%' height='100%' />
                        </SwiperSlide>
                        <SwiperSlide >
                            <p onClick={() => setappear(false)} className='position-absolute ' style={{ top: '2vh', right: '2vw', cursor: 'pointer' }}><IoIosCloseCircleOutline size={45} /></p>
                            <img src={item.image[1].url} alt="img" width='100%' height='100%' />
                        </SwiperSlide>
                    </Swiper>
                </motion.div>
                :
