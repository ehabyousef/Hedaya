...
export default function DashProd() {
    ...
    const delteProd = (id) => {
        axios.delete(`https://backend-kappa-beige.vercel.app/product/deleteProduct/${id}`)
            .then((res) => {
        ...
        <div className='container'>
            <div className="row my-5 row-gap-5" style={{ minHeight: "650px" }}>
                {visibleProducts.map((product, index) => (
                    <div className="col-12 col-md-4 col-lg-2 d-flex justify-content-center" key={index}>
                        <div style={{ width: '100%' }}>
                            <div className={css.content}>
                                <img onMouseLeave={() => sethoverd(false)} src={product.image[0].url} alt="hi" className={css.img} />
                                <div className={css.body}>
                                    <h3>{product.name}</h3>
                                    <h5 className={css.decrip}>{product.description}</h5>
                                    <span className="fs-4">
                                        {product.finalPrice}<span className="mx-2 fs-5" style={{ color: 'var(--brown)', textDecoration: 'line-through' }}>{product.price}</span>
                                    </span>
                                    <div className="d-flex align-items-center justify-content-center gap-3" id={css.view}>
                                        <div className="d-flex align-items-center gap-3" style={{ cursor: "pointer" }}>
                                            ...
                                            <h6 className="m-0 p-0" onClick={() => delteProd(product.id)}>Remove </h6>
                                        </div>
